...
const providerDefinitions = {
  claude: {
    ...
    workspaceIdEnv: "ANTHROPIC_WORKSPACE_ID",
  },
  gpt: {
    label: "ChatGPT",
    fn: askOpenAI,
...
function providerConfig(key) {
  ...
  const enabled = definition.local
    ? process.env.LOCAL_DISABLED !== "1"
    : Boolean(apiKey);
...
function resolveProvider(key, { includeLocal = true } = {}) {
  ...
  if (provider.local && !includeLocal) {
    throw new Error("The local Ollama provider is not available in this runtime.");
  }
...
export async function askAll({ prompt, providers, system, includeLocal = true }) {
  ...
  const keys = providers?.length
    ? providers
    : getProviders({ includeLocal }).filter((p) => p.enabled).map((p) => p.key);
...
export async function synthesize({
  ...
  if (!successful.length) throw new Error("No provider returned a successful response.");
  ...
  const synthesisPrompt =
    `You were given the same prompt sent to multiple AI models. Compare and synthesize their answers ` +
    `into one clear, useful response. Note meaningful disagreements when they matter.\n\n` +
...
export async function discuss({
  ...
  const system =
    `You're one participant in a discussion among multiple AI models on this topic: "${prompt}". ` +
    `You'll be shown what's been said so far. Respond naturally and specifically to previous points. ` +
    `Agree, disagree, or add a new angle. Keep it concise and don't merely restate prior responses.`;
  const transcript = [];
